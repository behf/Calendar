...
import com.example.behzad.wheel.ArrayWheelAdapter;
import com.example.behzad.wheel.NumericWheelAdapter;
import com.example.behzad.wheel.OnWheelChangedListener;
import com.example.behzad.wheel.WheelView;
...
public class DatePickerDailog extends Dialog {
    ...
    public DatePickerDailog(Context context, Calendar calendar,
		...
		// month
		int curMonth = calendarFarsi.getIranianMonth();
		String months[] = new String[] { "فروردین", "اردیبهشت", "خرداد",
				"تیر", "مرداد", "شهریور", "مهر", "آبان", "آذر",
				"دی", "بهمن", "اسفند" };
		month.setViewAdapter(new DateArrayAdapter(context, months, curMonth-1));
		month.setCurrentItem(curMonth-1);
		month.addChangingListener(listener);

		Calendar cal = Calendar.getInstance();
		// year
		int curYear = calendarFarsi.getIranianYear();
		int Year = calendarFarsi.getIranianYear();
		int curDay=calendarFarsi.getIranianDay()-1;
		...
		// day
		updateDays(year, month, day);
		day.setCurrentItem(calendarFarsi.getIranianDay()-1);
		...
		btnset.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				CalendarFarsi c = updateDays(year, month, day);
    ...
    CalendarFarsi updateDays(WheelView year, WheelView month, WheelView day) {
		...
		day.setViewAdapter(new DateNumericAdapter(Mcontex, 1, maxDays,calendarFarsi.getIranianDay()-1));
		int curDay = Math.min(maxDays, day.getCurrentItem() + 1);
		day.setCurrentItem(curDay - 1, true);
		calendar.set(Calendar.DAY_OF_MONTH, curDay);


		calendarFarsi.setIranianDate((calendarFarsi.getIranianYear()+(year.getCurrentItem()-NoOfYear)),month.getCurrentItem()+1,curDay);
		return calendarFarsi;
